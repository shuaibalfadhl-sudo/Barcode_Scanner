...
class _PrintScreenState extends State<PrintScreen> {
  ...
  void initState() {
    ...
    filteredToPrint = widget.allProducts.where((p) {
      // --- FIX: Support both Product and Bundle keys ---
      final barcode = (p['barcode'] ?? p['barcode_value'] ?? "").toString().trim();
      final sku = (p['product_code'] ?? p['bundle_sku'] ?? "").toString().trim();
  ...
  Future<void> _fetchBarcodeTypes() async {
    try {
      // --- FIX: Removed the 's' typo in the port number ---
  ...
  void _runFilter(String query) {
    setState(() {
      if (query.isEmpty) {
        ...
        displayedProducts = filteredToPrint.where((item) {
          // --- FIX: Support both Product and Bundle keys for searching ---
          final name = (item['product_name'] ?? item['bundle_name'] ?? "").toString().toLowerCase();
          final id = (item['product_id'] ?? item['id'] ?? "").toString().toLowerCase();
          final barcode = (item['barcode'] ?? item['barcode_value'] ?? "").toString().toLowerCase();
          final sku = (item['product_code'] ?? item['bundle_sku'] ?? "").toString().toLowerCase();
              ...
              build: (pw.Context context) {
                return pw.GridView(
                  ...
                  children: chunk.map((item) {
                    // --- FIX: Support both Product and Bundle keys for printing ---
                    final String barcode = (item['barcode'] ?? item['barcode_value'] ?? "").toString();
                    final String name = item['product_name'] ?? item['bundle_name'] ?? "Unknown";
                    final String sku = item['product_code'] ?? item['bundle_sku'] ?? "N/A";
      ...
      body: Column(
        children: [
          Container(
            ...
            child: Text("${selectedIndices.length} items selected for printing", style: TextStyle(color: const Color.fromARGB(255, 0, 0, 0), fontWeight: FontWeight.bold, fontSize: 12)),
          ),
          Expanded(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        ...
                        child: CheckboxListTile(
                          ...
                          // --- FIX: Support both Product and Bundle keys in the UI ---
                          title: Text(item['product_name'] ?? item['bundle_name'] ?? "Unnamed", style: const TextStyle(fontWeight: FontWeight.bold)),
                          subtitle: Wrap(
                            spacing: 6,
                            children: [
                              _miniBadge("SKU: ${item['product_code'] ?? item['bundle_sku']}"),
                              _miniBadge("BC: ${item['barcode'] ?? item['barcode_value']}"),
                              _miniBadge("TYPE: $typeDisplay"),
                              _miniBadge("ID: ${item['product_id'] ?? item['id']}"),
                            ],
                          ),
                          value: isSelected,
